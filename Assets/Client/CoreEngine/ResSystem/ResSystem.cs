...
public class ResSystem
{
    ...
    {
        AsyncOperationHandle<T> _handle = Addressables.LoadAssetAsync<T>(assetName);
    ...
    /// <param name="assetPath">一般是lable</param>
    /// <param name="handle">用来Release时使用</param>
    /// <param name="callBackOnEveryOne">注意这里是针对每一个资源的回调</param>
    /// <returns>所有资源</returns>
    public static IList<T> LoadAssets<T>(string assetPath, out AsyncOperationHandle<IList<T>> handle,
        Action<T> callBackOnEveryOne = null) where T : UnityEngine.Object
    {
        handle = Addressables.LoadAssetsAsync<T>(assetPath, callBackOnEveryOne, true);
    ...
    public static void LoadAssetsAsync<T>(string assetPath, Action<AsyncOperationHandle<IList<T>>> callBack,
        Action<T> callBackOnEveryOne = null) where T : UnityEngine.Object
    {
        Addressables.LoadAssetsAsync<T>(assetPath, callBackOnEveryOne).Completed += callBack;
    }
